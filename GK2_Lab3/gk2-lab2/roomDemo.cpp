	...
	m_particles{ {-1.3f, -0.6f, -0.14f} }
{
	...
	sd2.MipLODBias = 0;
	m_samplerBorder = m_device.CreateSamplerState(sd2);
	...
	auto tmp = XMMatrixTranslation(0.8, 0, 0) * XMMatrixScaling(1, -3.0/4.0, 0) * XMMatrixRotationZ(XM_PI / 18) * XMMatrixTranslation(0.5, 0.5, 0);
	XMStoreFloat4x4(&tempMtx, tmp);
...
void RoomDemo::DrawWalls()
{
	...
	XMStoreFloat4x4(&texMtx, XMMatrixScaling(0.25, 0.25, 1.0) * XMMatrixTranslation(0.5, 0.5, 0));
	...
	XMStoreFloat4x4(&texMtx, XMMatrixTranslation(0.5, 8, 0) * XMMatrixScaling(0.25, 4.0, 1.0));
	UpdateBuffer(m_cbTex1Mtx, texMtx);
...
void RoomDemo::Render()
{
	...
	XMFLOAT4X4 envProj = m_envMapper.FaceProjMtx();
	UpdateBuffer(m_cbProjMtx, envProj);
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		XMMATRIX faceView = m_envMapper.FaceViewMtx(static_cast<D3D11_TEXTURECUBE_FACE>(i));
		UpdateCameraCB(faceView);
		DrawScene();
		m_envMapper.SaveFace(m_device.context(), static_cast<D3D11_TEXTURECUBE_FACE>(i));
	}
