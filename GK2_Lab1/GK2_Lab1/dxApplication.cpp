...
void DxApplication::Update()
{
	...
	if (QueryPerformanceFrequency(&li)) {
		float freq = static_cast<float>(li.QuadPart) / 1000.0;
		m_angle += fmodf(freq * DirectX::XM_PI / 4, 2 * DirectX::XM_2PI);
	}
...
void DxApplication::Render()
{
	...
	float old_angle = m_angle, old_transX = m_cubeTransX;
	m_angle = 0;
	m_cubeTransX = 10.0f;
	Update();
	m_device.context()->DrawIndexed(36, 0, 0);
